...
export async function userActionsHandler(ctx: BotContext) {
  ...
  if (action === 'toggle_confirm') {
    ...
    // Validate ctx.from and ctx.from.id
    if (!ctx.from || !ctx.from.id) {
      return ctx.answerCallbackQuery('User context unavailable')
    }
    ...
    await redis.setex(`confirm:user_toggle:${userId}:${targetId}`, 300, 'pending')
  ...
  if (action === 'role') {
    ...
    // 2. Prevent demoting the last Super Admin (atomic transaction)
    if (newRole !== Role.SUPER_ADMIN) {
      try {
        // Use transaction to atomically check and update
        await prisma.$transaction(async (tx) => {
          const userToDemote = await tx.user.findUnique({ where: { telegramId: targetId } })
          if (userToDemote?.role === Role.SUPER_ADMIN) {
            const superAdminCount = await tx.user.count({ where: { role: Role.SUPER_ADMIN, isActive: true } })
        ...
        if (error instanceof Error && error.message === 'Cannot demote last super admin') {
      ...
      // If promoting to SUPER_ADMIN, no race condition risk
      await prisma.user.update({
        where: { telegramId: targetId },
        data: { role: newRole },
      })
...
async function showUserProfile(ctx: BotContext, telegramId: bigint) {
  ...
  if (telegramId.toString() !== ctx.from?.id.toString()) {
    ...
    for (const role of roles) {
      if (user.role !== role) {
        keyboard.text(ctx.t(`role-${role.toLowerCase().replace(/_/g, '-')}`), `user:role:${telegramId}:${role}`)
      }
    }
    keyboard.row()
  }
  ...
  const text = ctx.t('profile-display', {
    ...
    role: ctx.t(`role-${user.role.toLowerCase().replace(/_/g, '-')}`),
