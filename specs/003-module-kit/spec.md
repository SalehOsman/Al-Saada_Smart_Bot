...
- **FR-008**: System MUST provide draft middleware that automatically saves conversation state to Redis after each user input and resets the sliding TTL. Draft TTL (24h) controls Redis data retention — draft deleted after 24h of no interaction. Conversation timeout (15 min) controls grammY handler release — conversation exits after 15 min inactivity but draft remains in Redis. Both apply independently. TTL resets on every user interaction (sliding window).
