...
- **AdminScope**: Permission assignments for admins (sections/modules access). Attributes: `id` (PK, auto-generated), `userId` (FK→User), `sectionId` (FK→Section), `moduleId` (FK→Module, nullable — when null, grants access to entire section; when set, grants access to specific module only), `createdAt`, `createdBy` (FK→User, the Super Admin who assigned the scope).
