...
1. **Given** I am the user with the Telegram ID matching `INITIAL_SUPER_ADMIN_ID` in `.env` AND there are no existing Super Admins in the system **When** I send /start for the first time **Then** I am prompted to provide my Full Name and National ID, my account is created with the SUPER_ADMIN role, and I see a welcome message in Arabic. An optional nickname can be provided, otherwise it is auto-generated by taking my first name and appending a 4-character random slug (e.g., "Ahmed-x7y2").
...
1. **Given** I am a new user (not the super admin) **When** I send /start **Then** I am prompted to submit a join request by providing my Full Name, Phone Number, and National ID. An optional nickname can be provided, otherwise it is auto-generated (First Name + 4-char slug). My request is saved with status "PENDING" and all Super Admins receive a notification.
...
- **FR-034**: System MUST validate Egyptian phone number format during join request submission and reject invalid formats. Valid format: 11 digits, starting with one of the following operator prefixes:
  ...
  Regex: `/^(010|011|012|015)\d{8}$/`. Validation is handled by `@al-saada/validators` package ([egyptianPhoneNumber()](cci:1://file:///f:/_Al-Saada_Smart_Bot/packages/validators/src/phone.ts:4:0-22:1) in [packages/validators/src/phone.ts](cci:7://file:///f:/_Al-Saada_Smart_Bot/packages/validators/src/phone.ts:0:0-0:0)). Error message must be shown in Arabic.
...
- Q: Should auto-generated nicknames be unique across all users? → A: Yes. Auto-generated nickname format is `{firstName}-{4-char-nanoid}`. The 4-character nanoid provides sufficient collision resistance (~36^4 = 1.7M combinations). No uniqueness DB constraint is required for nicknames — collisions are acceptable and statistically negligible for ≤200 users.
