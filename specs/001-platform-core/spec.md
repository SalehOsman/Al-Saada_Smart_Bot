...
1. **Given** I am the user with the Telegram ID matching `INITIAL_SUPER_ADMIN_ID` in `.env` AND there are no existing Super Admins in the system **When** I send /start for the first time **Then** I am prompted to provide my Full Name and National ID, my account is created with the SUPER_ADMIN role, and I see a welcome message in Arabic. My nickname is auto-generated by taking my first name and appending a 4-character random slug (e.g., "Ahmed-x7y2").
...
- **FR-014 (Bootstrap Lock)**: The system MUST evaluate the `INITIAL_SUPER_ADMIN_ID` environment variable ONLY IF the `User` table contains exactly zero users with the `SUPER_ADMIN` role. Once at least one `SUPER_ADMIN` exists in the database, the `.env` verification is permanently disabled to prevent race conditions and backdoor access.
