...
- [x] T083 [P] Create input validation and sanitization utilities in `packages/validators/src/` (Zod schemas, XSS sanitization, and explicit `egyptianPhoneNumber` in `packages/validators/src/phone.ts` and `egyptianNationalId` in `packages/validators/src/national-id.ts` extraction functions). Unit tests MUST be in `packages/validators/tests/` covering phone regex edge cases and national ID century/birthdate/gender extraction (Constitution Principle IV).
- [x] T083-B [P] Write unit tests for Egyptian validators: `packages/validators/tests/phone.test.ts` and `packages/validators/tests/national-id.test.ts` — test valid/invalid phone prefixes (010/011/012/015), national ID century extraction (2=1900s, 3=2000s), birthdate parsing, gender detection (odd=male, even=female).
...
- [x] T025-B [US2] Verify National ID extraction and validation (FR-035) and Phone validation (FR-034) with unit tests. Extraction MUST verify: century digit (2=1900s, 3=2000s), birthdate (digits 2-7 → YYMMDD), gender (digit 9: odd=MALE, even=FEMALE). Tests in `packages/validators/tests/national-id.test.ts`.
- [x] T026 [US2] Save join request to database with PENDING status using `joinRequestService` in `packages/core/src/services/join-requests.ts`. Generate a short human-readable request code via nanoid (e.g., `REQ-A1B2C3`) for display in admin notifications.
...
- [x] T107 [P] [US6] Implement System Info Display: read-only view (bot version, uptime, connected services status, environment). Service status indicators MUST use health check logic from T077 (PostgreSQL `$queryRaw SELECT 1`, Redis `ping()`) to show real-time connectivity status.
...
- [x] T108 [P] [US6] Implement Backup (Full Control): trigger DB backup (pg_dump), download via `ctx.replyWithDocument` (sends .sql file to Super Admin in Telegram chat), view history, restore. Backup files MUST use naming convention `backup-YYYY-MM-DD-HHmmss.sql` (spec US6 scenario 4). The double-confirmation prompt for restore MUST use i18n key `backup-restore-confirm` (NOT hardcoded text). The confirmation keyword that the user must type MUST be read from i18n key `backup-restore-confirm-keyword` — this allows Arabic/English versions (e.g., "تأكيد" / "CONFIRM") without hardcoded strings (Constitution Principle VII).
...
- [x] T059 [P] [US5] Create audit log service in `packages/core/src/services/audit.ts` implementing the 25 platform-core auditable actions defined in spec.md FR-026 (28 total in Prisma enum; the additional 3 MODULE_CREATE/UPDATE/DELETE are from 003-module-kit). Each log entry: `{ userId: bigint, action: AuditAction, targetType?: string, targetId?: string, details?: Json }`. Complete 001-platform-core action list: `USER_BOOTSTRAP`, `USER_LOGIN`, `USER_LOGOUT`, `ROLE_CHANGE`, `USER_APPROVE`, `USER_REJECT`, `USER_ACTIVATE`, `USER_DEACTIVATE`, `JOIN_REQUEST_SUBMIT`, `SECTION_CREATE`, `SECTION_UPDATE`, `SECTION_DELETE`, `SECTION_ENABLE`, `SECTION_DISABLE`, `MODULE_REGISTER`, `MODULE_UNREGISTER`, `MODULE_ENABLE`, `MODULE_DISABLE`, `MAINTENANCE_ON`, `MAINTENANCE_OFF`, `PERMISSION_CHANGE`, `ADMIN_SCOPE_ASSIGN`, `ADMIN_SCOPE_REVOKE`, `BACKUP_TRIGGER`, `BACKUP_RESTORE`
