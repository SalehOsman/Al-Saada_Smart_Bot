...
- [ ] T097 [P] Sentry Integration (Constitution Principle XI — Observability): Configure Sentry error tracking opt-in via `SENTRY_DSN` env var. When set, integrate `@sentry/node` into the grammY error handler. MUST enable PII filtering (`sendDefaultPii: false`) and strip `nationalId`/`phone` from breadcrumbs. When `SENTRY_DSN` is unset, Sentry is disabled (no-op). This is NOT a blocker for v0.1.0 — it's additive for production readiness.
